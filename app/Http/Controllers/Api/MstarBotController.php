...
class MstarBotController extends Controller
{
    ...
    {
        $result = $this->miraBotRepository->getAll();
    ...
    {
        // Build URL
        $handlerBackUrl = route('MstarBot.handleEvent');
        ...
        if ($request->input('data.PARAMS.CHAT_ENTITY_TYPE') == 'LINES') {
            ...
            switch ($message) {
                ...
                case '0':
                    ...
                    break;
                default:
                    //Gọi APi để lấy Conversation ID
                    // $responseData = $this->createConversationId();
                    // $conversation = MstarBotConversation::where('dialog_id', $request->input('data.PARAMS.DIALOG_ID'))->first();
                    $conversation = $this->miraBotRepository->findByDialogId($request->input('data.PARAMS.DIALOG_ID'), $request->input('auth.domain'));
                    $conversationId = $conversation->conversation_id;
                    $tokenBot = $conversation->token_bot;
                    $response = $this->miraBotRepository->callMiraAPI($conversationId, $tokenBot, $request->input('data.PARAMS.MESSAGE'));
                    ...
                    $this->mstarBotMessageLogRepository->create($saveData);
                    break;
            }
        } else {
            // $conversation = MstarBotConversation::where('dialog_id', $request->input('data.PARAMS.DIALOG_ID'))->first();
            ...
            if (!$conversation) {
                Log::info('Không tìm thấy Conversation', [$conversation]);

                $this->miraBotRepository->createConversationId();
                return $this->handleMessageAddEvent($request, $clientId, $clientSecret);
            }
    ...
    {
        //Gọi APi để lấy Conversation ID
        $responseData = $this->miraBotRepository->createConversationId();
        Log::info("Join message", [$responseData]);
        $token_bot = $responseData['data']['token'];
        ...
        $this->miraBotRepository->create($mstarBotConversation);
